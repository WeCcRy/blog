...
> **为什么 reactive 对象在解构时会丢失响应式，而使用 toRefs 解构不会？**
> 
>- reactive 解构丢失响应式的原因：reactive 基于 Proxy 实现代理。解构对象时，如果提取的是基本数据类型（如数字、字符串），在 JavaScript 中这相当于值拷贝。解构出来的独立变量脱离了 Proxy 对象的上下文，后续对其读取或修改都不会再经过 Proxy 拦截，因此响应式系统无法追踪它。
> 
>- toRefs 保留响应式的原理：toRefs 会遍历原 reactive 对象，将它的每个属性都包装成一个 ref 对象。解构 toRefs 的结果时，提取出来的是一个个具体的 ref 对象（引用传递）。当我们访问解构变量的 .value 时，这个特殊的 ref 对象底层依然会去读取或修改原 reactive 对象的对应属性，从而保持了响应式连接。
