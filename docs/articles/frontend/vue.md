...
Vue 3 使用了**快速 Diff 算法**，相较于 Vue 2 的双端 Diff 算法，通过引入“最长递增子序列”最大程度地减少了 DOM 移动操作，性能有了显著提升。其核心步骤如下：

1. **同步头部节点**：从新旧虚拟节点的头部开始比较，如果是相同节点（`key` 和 `type` 相同），则直接进行 `patch` 更新并向后移，直到遇到不同节点。
2. **同步尾部节点**：从新旧虚拟节点的尾部开始比较，同样进行 `patch` 更新并向前移，直到遇到不同节点。
3. **新增或卸载边界节点**：
   - 如果经过头尾处理后，旧节点已经遍历完，但新节点还有剩余，说明这些是**新增**节点，直接将其挂载。
   - 如果新节点已经遍历完，但旧节点还有剩余，说明这些是**废弃**节点，直接将其卸载。
4. **处理未知序列（核心环节）**：如果头尾处理完后，新旧节点序列中间都有剩余（即存在乱序节点），则会进行以下计算以将 DOM 移动降至最低：
   - 根据新节点的 `key` 建立一个「键 -> 索引」对应的映射表。
   - 遍历旧节点的乱序部分，在映射表中寻找对应可复用的新节点。如果找不到则卸载旧节点；如果找到则进行 `patch` 并且在 `newIndexToOldIndexMap` 中记录该节点的新旧位置映射。
   - **计算最长递增子序列 (Longest Increasing Subsequence)**：基于上一步收集到的位置数组映射，计算出一条最长递增子序列。在这个序列中的元素，它们在新旧列表中的相对先后顺序是没有改变的。因此，**在最长递增子序列中的节点不需要进行 DOM 移动操作**，只需把不在该子序列中的节点进行移动和插入即可。

   > **举例说明最长递增子序列如何显着减少 DOM 移动：**
   > 假设经过头尾比对剔除后，剩下了：
   > - **旧乱序节点**：`b, c, d`
   > - **新乱序节点**：`c, d, b`
   > 
   > 1. Vue 会遍历新序列 `c, d, b`，记录它们在旧序列中的位置索引，生成一个位置数组映射：`[2, 3, 1]`（即 `c` 原在索引2，`d` 原在索引3，`b` 原在索引1）。
   > 2. 对数组 `[2, 3, 1]` 求最长递增子序列，得到结果 `[2, 3]`（对应的节点正是 `c` 和 `d`）。
   > 3. **最终操作**：因为 `c` 和 `d` 在递增子序列中，证明它们的相对顺序是正确的，**无需作任何 DOM 移动**。Vue 只需要挑出不在子序列中的 `b`，把它移动到 `d` 的前面即可。通过这一算法，原本需要多次的挪动被极致压缩成了一次。
...
toRef，一般用于提取响应式对象中的某个值，并保持响应式连接。
...
fooRef.value++
console.log(state.foo) // 2 (源对象也会跟着改变)
```

toRefs，批量提取响应式对象中的所有属性，常用于将 reactive 对象解构时保持对其属性的响应式连接。

```javascript
const state = reactive({ foo: 1, bar: 2 })
// 解构后提取出的 foo、bar 都是独立的 ref 对象
