...
> 什么是块级作用域？和函数作用域的区别是什么？
> 块级作用域是由一对花括号 `{}` 包围的代码块形成的作用域，变量在这个代码块内有效，外部无法访问。函数作用域是由函数定义形成的作用域，变量在函数内部有效，外部无法访问。块级作用域更细粒度，可以在函数内部创建多个独立的作用域，而函数作用域只能创建一个作用域。var声明的变量是函数作用域，而let和const声明的变量是块级作用域。
