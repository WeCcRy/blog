...
闭包是 JavaScript 中最核心的概念之一。简单来说，**“闭包就是能够读取其他函数内部变量的函数”**。

### 1. 定义
在 JavaScript 中，每当创建一个函数，闭包就会在函数创建阶段被同时创建出来。闭包由两部分组成：
- **函数本身**。
- **该函数被创建时所处的词法环境**（Lexical Environment）。

### 2. 核心特征
- **保留上下文**：内部函数可以访问外部函数声明的变量，即使外部函数已经执行完毕并从调用栈中弹出。
- **变量持久化**：被闭包引用的内部变量不会被垃圾回收机制（GC）销毁，直到闭包本身被销毁。

### 3. 应用场景
- **封装私有变量**：防止外部直接修改函数内部状态。
- **柯里化 (Currying)**：预置函数参数。
- **沙箱隔离 (本文乾坤沙箱示例)**：利用形参优先级覆盖全局作用域，将代码限制在局部环境内运行。

### 4. 乾坤沙箱中的闭包逻辑
回顾沙箱示例：`((window) => { ... })(proxy)`
1. **匿名函数**是内部函数。
2. **传入的 `proxy`** 是它能访问到的“外部”变量。
3. 在函数体内，所有的代码都处于这个闭包的作用域下。根据 JS 的作用域链规则，程序会**由近及远**查找变量。
4. 因为闭包参数里定义了 `window`，所以代码内部访问的 `window` 会直接锁定在 `proxy` 上，从而实现对真实全局 `window` 的屏蔽。
