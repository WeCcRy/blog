...
function quickSort(arr) {
    // 辅助函数：负责实际排序，传入数组和要排序的范围（low到high）
    function sortHelper(arr, low, high) {
        // 如果范围里只有一个元素或没有元素，就不用排了
        if (low >= high) return;

        // 选第一个元素当标杆（temp），此时 low 位置产生了一个坑位
        const temp = arr[low];
        let i = low;
        let j = high;
        ...
        // 递归处理左子数组
        sortHelper(arr, low, i - 1);
        // 递归处理右子数组
        sortHelper(arr, i + 1, high);
    }
...
* 独立调用的函数（不通过对象调用，如直接调用`fn()`）在非严格模式下 this 指向全局对象，在严格模式下 this 为 undefined
...
- ```js
  // 深拷贝的实现，简易版。对于Map,Set等类型会转成普通数组，无法处理循环引用
...
- ```js
  ...
  function deepCopy(object, hash = new WeakMap()) {
    ...
    if (object instanceof Date) {
      ...
      hash.set(object, newDate);
      return newDate;
    }

    // 处理正则对象
    if (object instanceof RegExp) {
      const newReg = new RegExp(object.source, object.flags);
      newReg.lastIndex = object.lastIndex; // 保留正则的lastIndex属性
      hash.set(object, newReg);
      return newReg;
    }
    ...
    if (object instanceof Map) {
      ...
      hash.set(object, newMap); // 先缓存，避免循环引用
    ...
    if (object instanceof Set) {
      ...
      hash.set(object, newSet); // 先缓存，避免循环引用
    ...
    hash.set(object, newObject); // 缓存当前对象，解决循环引用

    // 遍历所有自身属性（包括Symbol键）
    Reflect.ownKeys(object).forEach(key => {
      newObject[key] = deepCopy(object[key], hash);
    });
